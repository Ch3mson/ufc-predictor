...
A client built on the UFC predictor microservice available in the [ufc-predictor repository](https://github.com/Ch3mson/ufc-predictor). This app allows anyone on the web to access the ML model that I created to predict on upcoming UFC fights, for both men and women. For best results, choose fighters that are in the same weight division and gender, since the model is trained on fight stats, and doesn't take into account of weight/gender. On startup, it may take a few seconds since Google Cloud Run needs to do a cold start when it is inactive.
